...
class Cipher:
	...
	def encode_otp(self, message):
		...
		# save the length of the key and message bit strings into self.str_len
		# this is needed because if there are leading zeroes, conversion into a number will drop them
		# and they will need to be added back
		self.str_len = len(binary_string)
		...
		cipher_text = ''.join(cipher_text)
		cipher_text = binary_string_to_binary(cipher_text)
	...
	def decode_otp(self, cipher_text, key):
		...
		cipher_text = binary_to_binary_string(cipher_text)
		# Adding dropped leading zeroes back to key (see comment in encode_otp)
		if len(key) != self.str_len:
			difference = abs(len(key) - self.str_len)
			key_list = list(key)
			for i in range(difference):
				key_list.insert(0, "0")
			key = ''.join(key_list)
		# Adding dropped leading zeroes back to ciphertext
		if len(cipher_text) != self.str_len:
			difference = abs(len(cipher_text) - self.str_len)
			cipher_list = list(cipher_text)
			for i in range(difference):
				cipher_list.insert(0, "0")
